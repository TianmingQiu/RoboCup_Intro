                                                    ...
                                                    const Vector2f& plWorld, const Vector2f& prWorld, float goalpostRadius)
{
    ...
    Matrix4f coefficientMatrix;
    coefficientMatrix << float(plWorld.x()),float(plWorld.y()),1,0,
                        -float(plWorld.x()),float(plWorld.y()),0,1,
                        float(prWorld.x()),float(prWorld.y()),1,0,
                        -float(prWorld.x()),float(prWorld.y()),0,1;
    
    
    Vector4f b(pl.x(),pl.y(),pr.x(),pr.y());
    
    Vector4f Pose_Vector;
    Pose_Vector = coefficientMatrix.inverse() * b;
    
    
    Pose2f result;
    result.translation.x() = Pose_Vector(2);
    result.translation.y() = Pose_Vector(3);
    result.rotation = std::atan2(Pose_Vector(1),Pose_Vector(0));
  return result;
